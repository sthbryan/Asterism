...
fn configure(dir: &Path) {
    assert!(git(dir, &["config", "user.email", "test@example.com"])
        .status
        .success());
    assert!(git(dir, &["config", "user.name", "asterism-test"])
        .status
        .success());
}
...
fn peer_commit(root: &Path, name: &str, content: &str) {
    ...
    assert!(git(&peer, &["push", "-q", "origin", "HEAD:main"])
        .status
        .success());
    let _ = fs::remove_dir_all(&peer);
}
...
fn dirty_worktree_counts_and_blocks_switch() {
    ...
    assert_eq!(
        switch_branch_dir(&work, "feature").unwrap_err(),
        "GIT_DIRTY"
    );
    assert_eq!(
...
fn fast_forward_pull_applies_remote_commits() {
    ...
    assert_eq!((status.ahead, status.behind), (Some(0), Some(0)));
    assert_eq!(
        fs::read_to_string(work.join("peer.txt")).unwrap(),
        "from peer\n"
    );
    let _ = fs::remove_dir_all(root);
}
...
fn push_without_upstream_then_set_upstream() {
    ...
    assert_eq!(
        push_dir(&work, "origin", false).unwrap_err(),
        "GIT_NO_UPSTREAM"
    );
...
fn non_git_directory_is_rejected() {
    ...
    assert_eq!(sync_status_dir(&root).unwrap_err(), "GIT_NOT_GIT");
    let _ = fs::remove_dir_all(root);
}
